...
| 31 | **(P4)** 前台 stage settledTrack 去抖 + 重渲染收敛(QA#46 burst 真因) | ✅ Completed(QA#47:cadence < ~600ms 已收敛,且优于 tab-2) | [Phase 31](#phase-31p4-前台-stage-settledtrack-去抖--重渲染收敛) |
| 32-A | AutoScrollText 只在内容/尺寸变化时重测(QA#49 回流风暴) | ✅ 代码完成(待 32-B clean 复测) | [Phase 32-A](#phase-32-a-autoscrolltext-只在内容尺寸变化时重测-代码完成) |
| 32-B | **(carve-out)** worst-case GC 残留 clean-prod 归因与收尾 | 🔄 移交 [20260615-now-playing-switch-gc-closure PRD](../20260615-muzero-now-playing-switch-gc-closure-prd/20260615-muzero-now-playing-switch-gc-closure-prd.md) | 见该 PRD |
...
| P1 | `510f7c9` | Replace diagnostic cover-color freezes with a production settled/discrete color update | 当前稳定基线;P2/P3 未改善主观流畅度,已回滚 |
| P2 | `17941b3` / reverted | Cache MediaSession artwork object URLs by coverBlobId | cache-hit 生效但主观流畅度未改善 |
| P3 | `93a54d9` / reverted | Schedule normal MediaSession metadata/artwork instead of awaiting before play | trace 显示 schedule 生效但仍不流畅 |
...
- [x] P1 production fix:remove diagnostic color freezes and implement settled/discrete cover color updates;current baseline restored to `510f7c9`.
- [x] P2/P3 rollback:revert MediaSession artwork cache/schedule changes after QA trace showed no subjective smoothness improvement.
...
### QA#35(2026-06-14 A6c trace):跳过 color apply 后 FPS 恢复

17:37 trace 是 `5f62787 diag(perf): isolate cover color apply` 的结果:

- **FPS 恢复**:trace 中多段切歌窗口保持 `fpsAvg≈105.9~116.1`, `fpsLow≈17.1~20`, `frameMaxMs≈50~58.4ms`;没有复现 A6b 的 `fpsAvg≈55~79` 下沉。
- **palette read/log 继续存在**:每次切歌仍出现 `cover.palette.track-metadata paletteCount=4`,同时出现 `cover.palette.apply phase=skip reason=diag-bisect`。因此 palette liveQuery/metadata 读取/日志本身不是掉帧根因。
...
### QA#37(2026-06-14 A6e trace):跳过 rAF 后仍未恢复

17:54 trace 是 `5de774e diag(perf): isolate cover color raf updates` 的结果:

- **rAF 插值确实被跳过**:每次切歌都有 `cover.palette.css phase=skip reason=diag-bisect` 和 `cover.palette.transition phase=skip reason=diag-bisect`,同时仍有 `cover.palette.track-metadata paletteCount=4`。
- **FPS 仍回退**:开始窗口可达 `fpsAvg≈110.2~115.5`,但 rapid switch 中降到 `95.6 -> 87.8 -> 81.2 -> 86.1 -> 93.5`,`fpsLow≈10.9~15`;结束窗口回到 `106.9/fpsLow=17.1`。这仍弱于 A6c 的稳定 `105.9~116.1/fpsLow≈17~20`。
- **Pixi 小图仍不是瓶颈**:texture 仍是 `192×192 image/webp`,大多数 `fetch≈0.9~1ms`, `decode≈0.9~1.2ms`, `apply≈0.7~0.9ms`。首个 `apply≈4.7ms` 仍远低于 frameMax `91.6ms`,不解释整体窗口下沉。
- **结论**:即便没有 CSS 输出和 900ms per-frame store updates,单次 `setState({ rgb, palette })` 后的消费者行为仍足以造成回退。当前最可疑的是 flow 背景每帧通过 `getVisualizerCoverPalette()` 读 palette 并上传 `uColors`;spectrum 则通过 `getVisualizerCoverColorRgb()` 读 rgb。
- **下一刀(A6f)**:继续冻结 CSS/rAF,但只更新 `rgb`,冻结 `palette`。若 A6f 恢复,正式修复应优先让 flow palette 离开切歌热路径或在落定/低频更新;若仍回退,再拆 rgb/spectrum 或 color-store update 本身。

### QA#38(2026-06-14 A6f trace):冻结 palette 后仍未完全恢复

17:58 trace 是 `795b9a3 diag(perf): isolate cover color palette output` 的结果:

- **palette 输出确实被冻结**:每次切歌都有 `cover.palette.palette phase=skip reason=diag-bisect`,且 CSS/rAF 也继续被 skip;`cover.palette.track-metadata paletteCount=4` 仍保留。
- **FPS 只部分改善**:窗口主要在 `fpsAvg≈95.6~99.5`, `fpsLow≈15~20`, `frameMax≈50~58.3ms`;比 A6e 的最低 `fpsAvg≈81.2/fpsLow≈10.9` 好,但仍没有回到 A6c 的稳定 `fpsAvg≈105.9~116.1`。
...
### QA#40(2026-06-14 A6h trace):跳过 color store setState 没有继续改善

18:06 trace 是 `67cc03c diag(perf): isolate cover color store update` 的结果:

- **A6h 生效**:每首都有 `cover.palette.store phase=skip reason=diag-bisect`,同时 `css/palette/rgb` 也继续 skip;`track-metadata paletteCount=4` 仍保留。
- **没有比 A6g 更好**:FPS 窗口为 `fpsAvg≈94.3~96`, `fpsLow≈17.1~20`, `frameMax≈50~58.4ms`。这没有贴近 A6c 的 `105.9~116.1`,说明 A6g 里残留的 `coverBlobId` store setState 不是关键成本。
...
### QA#44(2026-06-14 P3 trace):MediaSession 延后仍不流畅,回滚到 `510f7c9`

18:32 trace 是 `93a54d9 fix(perf): schedule media session artwork` + `17941b3 fix(perf): cache media session artwork` 之后的新结果;QA 反馈“还是没有更流畅”,建议回到 `510f7c9c642fea75f06b41aa0045f5a8f92f7a25`。

- **P3 机制生效但主观收益不足**:trace 里 `player.mediaSession metadata.schedule phase=start/skip/success` 可见,说明 normal path 已经不再立即 await MediaSession artwork。但窗口仍有 `fpsAvg≈99.1 -> 86.4 -> 80.6 -> 82.8 -> 96.4`, `fpsLow≈12~13.3`。
- **MediaSession 已不是本轮主要阻塞点**:最终 `metadata.schedule phase=success` 后才出现 `player.mediaSession.artwork.fetch≈19.8ms`,位置已在切歌窗口后段;但切歌窗口仍不够顺。
- **Pixi 数字仍需按 settle 解读**:`background.pixi media.load≈190~223ms` 包含已有 settle delay;真实 `background.texture fetch/decode` 多数为 `≈1~5ms`,个别 `fetch≈27.4ms`, `decode≈17ms`;`textureSwap.apply≈0.6~0.9ms`。
- **回滚决策**:P2/P3 属于“机制正确但没有提升体验”的修补,继续叠加会污染基线。执行 revert,代码回到 `510f7c9` 的 P1 stable baseline,后续优化应从该点重新开分支/重新二分,优先关注全局背景合成/cover preload cadence,而不是继续扩展 MediaSession 路径。

### QA#45(2026-06-15 tab-1 vs tab-2 切歌对比):剩余差距在 Now Playing 前台,不在全局背景

用户提供同一 commit(`be2462c`)、同一会话、同一批歌、同样狂按 next/prev 的两份 trace,只差「停在哪个 tab」:

- **tab-2(队列/库,Dock 或快捷键切歌)**:[`.logs/commit-be2462c.../tab-2-switch-song.log`](../../../.logs/commit-be2462c3b09aa91b978143156f36747215a12c96/tab-2-switch-song.log)
- **tab-1(Now Playing,页内切歌)**:[`.logs/commit-be2462c.../tab-1-switch-song.log`](../../../.logs/commit-be2462c3b09aa91b978143156f36747215a12c96/tab-1-switch-song.log)

**关键数据(`performance.frame` 窗口):**

| 指标 | tab-2(其它 tab) | tab-1(Now Playing) | 解读 |
|---|---|---|---|
| `fpsAvg`(切歌中) | **~106–110**(109.1 / 105.4 / 111.9 / 106.4) | **~99**(112.5 → 99.1 → 98.6) | tab-1 平均**持续低 ~10 FPS** |
| `frameMaxMs` | 58.4(个别 116.6) | 50–58.3 | **两边同量级**(尖峰来自共享路径) |
| `longTaskMaxMs` | 166 | 166 | 完全相同(共享路径残留) |

**结论(本 PRD 的方向修正):**前 27 个 Phase 全部在追**全局共享的背景/Pixi/cover 管线**(挂在 [`App.tsx:232`](../../../src/App.tsx#L232) `NowPlayingBackground`,两个 tab 同一份)。本对比证明:

1. **尖峰(`frameMax`/`longTask`)两边相等** → 那次最重的单帧确实来自共享背景,与 tab 无关(印证前 27 Phase 的工作对象);
2. **但 `fpsAvg` 持续低 ~10 不是尖峰**,而是 tab-1 多了一份**贯穿全程、每帧都在花**的开销 → 只可能来自 **Now Playing 前台组件**(仅 tab-1 挂载),这是前 27 Phase **从未触及**的层。
3. `cover.palette.settle` 已是 650ms 防抖 + seq 去重([`visualizer-color-store.ts:64`](../../../src/stores/visualizer-color-store.ts#L64)),狂切只落最后一次,**不是元凶**(已排除)。

**前台开销定位(按对 `fpsAvg` 的贡献排序):**

- **P1 — `PlaybackSpectrum` 常驻 rAF(连续成本,因果最干净)**:仅在 Now Playing 挂载([`now-playing-page.tsx:135`](../../../src/pages/now-playing-page.tsx#L135));播放期间每帧 canvas 画几百个圆角矩形([`playback-spectrum.tsx:104-110`](../../../src/components/player/playback-spectrum.tsx#L104-L110)),即使不切歌也在跑。这是把基线从 ~110 压到 ~105 的稳定连续开销。
- **P2 — `SwipeableMediaStage` coverflow 切歌动画(连续 + 尖峰)**:每次外部切歌触发 [`swipeable-media-stage.tsx:403`](../../../src/components/player/swipeable-media-stage.tsx#L403) `playProgrammaticSwitch`,跑 0.62s Motion 动画 + portal 挂出 3 张 coverflow 卡(prev/current/next),每卡含全分辨率封面 `<img>` + 一张 `filter: blur(20px) saturate(400%)` `scale(1.12)` 背光图([`swipeable-media-stage.tsx:966-980`](../../../src/components/player/swipeable-media-stage.tsx#L966-L980)),跑在 `[perspective:1200px]` + `preserve-3d` + `rotateY/scale/opacity` 的 3D 合成层。日志里 tab-1 每次切歌的 `cover.render cache-hit` 明显多于 tab-2(6~7 vs 4),正是这 3 卡。**狂切间隔 ~0.5s < 动画 0.62s → overlay 从不空闲**,模糊+400% 饱和背光在 3D layer 上持续合成 = 既拉低平均、又是切歌尖峰来源。
- **P3 — 前台重挂 + 布局抖动(尖峰)**:`AnnotationEditor key={current.id}` 每首整体卸载重建([`now-playing-page.tsx:139`](../../../src/pages/now-playing-page.tsx#L139));overlay 可见时的 capture 阶段 `scroll`/`resize` 监听 → `updateOverlayRect` → `getBoundingClientRect` + `measureVerticalClipBounds`(向上遍历父节点读 `getComputedStyle`,[`swipeable-media-stage.tsx:218`](../../../src/components/player/swipeable-media-stage.tsx#L218) / [`:1032`](../../../src/components/player/swipeable-media-stage.tsx#L1032));叠加 Lenis 切歌滚动复位([`now-playing-page.tsx:73-76`](../../../src/pages/now-playing-page.tsx#L73-L76))= 强制同步布局(layout thrash)。

**测量方法学(沿用 §4 + 本 PRD 既有仪表):**目标指标 = `fpsAvg`(平均,本轮主信号)+ `frameP99/frameMax` + `longTaskMaxMs`;基线对比锚点 = 同 commit 的 tab-1 vs tab-2 两份 trace(消除会话/曲库差异);**每个 Phase 验收 = tab-1 切歌窗口 `fpsAvg` 向 tab-2 靠拢,且 `frameMax/longTask` 不恶化**。prod build(`make build` / 桌面)复测、第二轮取数(首轮 warmup 不算)。回退 = `git revert`,不藏 flag(硬规则 3)。

---

## 6bis. 落地方案(按 priority,QA#45 后续)

> 三个 Phase 互相独立、可分别 ship/验证;建议按 P1 → P2 → P3 顺序,因为 P1 改动最小、对 `fpsAvg` 的因果最干净,能先把"连续基线"差距证伪/坐实,再处理切歌瞬间的 P2/P3。

### Phase 28(P1): PlaybackSpectrum 切歌 fade + 停 rAF

**产品口径修正(2026-06-15,PM):**不做「降帧率」。改为**切歌时频谱 fade out 并完全停 rAF(rAF = 0)**,落定后再 fade in 恢复——「切歌过程中本来也没有音乐」,所以暂停比降帧更合理、收益更干净。

**Goal:** 切歌(尤其狂切 burst)期间彻底消除频谱的每帧 canvas 重画(连续成本),落定后无缝恢复。

**现状:** [`playback-spectrum.tsx`](../../../src/components/player/playback-spectrum.tsx) 的 rAF loop 在 `isPlaying || dragging` 时**每个浏览器帧**(~120Hz)都 `renderFrame()`,画数百个 `roundRect`,切歌期间照跑。

**Tasks(✅ 代码完成):**
- [x] 用 [`useSettledValue`](../../../src/hooks/use-settled-value.ts) 派生 `switching = current.id !== settledTrackId`(settle `SPECTRUM_SWITCH_SETTLE_MS=420`),狂切 burst 期间持续为 true。
- [x] 纯 helper `shouldAnimateSpectrum({isPlaying,dragging,switching})`:`dragging` 永远动(seek 跟手),否则 `isPlaying && !switching`;`switching` 时 **rAF 完全不调度**。
- [x] canvas `switching` 时 `opacity-0`(CSS `transition-opacity duration-300` fade out),落定 fade in;暂停态(settled)仍画单帧静态波形。
...
**Goal:** 狂切 burst 时不再让 coverflow overlay 持续合成 3D 模糊背光,把切歌窗口的 `fpsAvg` 拉回。

**现状:** [`playProgrammaticSwitch`](../../../src/components/player/swipeable-media-stage.tsx#L360) 每次切歌都全量播放 0.62s coverflow;`incomingReady` 只 gate 封面是否就绪([`:420`](../../../src/components/player/swipeable-media-stage.tsx#L420)),**不 gate 切歌速率**。背光层 `blur(20px) saturate(400%)` 是 compositor 重负。

**Tasks(✅ 代码完成):**
- [x] 外部切歌效果加**速率 gate**:`Date.now() - lastSwitchAtRef < COVERFLOW_BURST_SKIP_MS(600)` 即判 burst → 跳过 coverflow overlay + `clearStack()`,直接走底层 [`MediaStage`](../../../src/components/player/media-stage.tsx) 交叉淡入(`baseHidden` 本就切换)。非 burst(空闲后首切)仍播完整 coverflow。
- [x] burst 期间 `bursting` 状态把底层 stage 背光关掉(`coverBacklightEnabled={baseCoverBacklightEnabled && !bursting}`),`COVERFLOW_BURST_SETTLE_MS(500)` 静默后恢复 → 不再每首合成 `blur(20px) saturate(400%)`。
- [x] 不改单次(非 burst)切歌的 coverflow 观感(现有 6 例回归全绿)。
...
**Goal:** 去掉切歌瞬间的 React 重挂与强制同步布局,削平 tab-1 切歌尖峰。

**Tasks(✅ 代码完成):**
- [x] `AnnotationEditor` 去 `key={current.id}` 整体重挂([`now-playing-page.tsx`](../../../src/pages/now-playing-page.tsx));改为 `useEffect([track.id])` in-place reset 草稿(`tagInput`/`tagInputOpen`/`cropFile`),子组件 [`TrackMemoryNotesPanel`](../../../src/components/track/track-memory-notes-panel.tsx) 同样 `useEffect([trackId])` reset composer 草稿 → 保留「每首 fresh」UX,不再拆/建整棵子树 + masonry 重排。
- [x] `updateOverlayRect` 的 capture `scroll`/`resize` 监听 rAF 合并(一帧最多测一次 `getBoundingClientRect` + `measureVerticalClipBounds`),消除滚动期间的强制同步布局。
- [x] Lenis 切歌滚动复位确认仍 `immediate: true`([`now-playing-page.tsx`](../../../src/pages/now-playing-page.tsx)),且 overlay rect 测量已 rAF 合并,不再每个 scroll 事件同步 layout — 无需额外错峰。
...
**Tasks(✅ 代码完成):**
- [x] 新增 [`useBurstSettledValue`](../../../src/hooks/use-burst-settled-value.ts)(leading-edge 即时 + trailing 去抖,区别于 `useSettledValue` 的纯 trailing):单切落 leading 即时,只有真·狂按(间隔 < `quietMs`)才合并,跳过中间曲。
- [x] [`MediaStage`](../../../src/components/player/media-stage.tsx) 封面/标题视觉跟 `useBurstSettledValue(current, 300)`(video 元素 source 仍跟 live track,由 player-store 驱动)。
- [x] `memo` 化 track-无关前台叶子(`TransportControls` / `PlaybackSpectrum` / `NowPlayingActionRow` / `TrackInfoCard`)→ `currentIndex` 变化不再经 `NowPlayingPage` 父级级联重渲染它们。
- [~] `StageIdentity` 未单独 settle/memo(在 `SwipeableMediaStage` 内,随其 currentIndex selector 重渲染);`NowPlayingPanel` 右栏未复核 → 留 QA#47 后评估,实测已收敛故未追加。
- [x] 单切即时性不变:leading edge 即时应用(单测覆盖)。

### Phase 31 Checklist

- [x] 单测:`useBurstSettledValue` leading/trailing/burst-coalesce/return-to-leading(`use-burst-settled-value.test.tsx`,5 例绿)。
- [ ] ⚠️ tab-1 每切歌 `cover.render` 仍 ~7(QA#47:42 条全 cache-hit)——**该指标作废**:cache-hit 是廉价的,实测 fps 已收敛,证明 cover.render 计数不是瓶颈代理。
- [x] **tab-1 切歌 `fpsAvg` 向 tab-2 收敛(QA#47,~490–590ms cadence):tab-1 `fpsAvg 108` ≈ tab-2 ~110,`frameMax 41.7 < 75`,`fpsLow 24 > 13.4` —— 达成(略优于 tab-2)。**
- [x] 现有 now-playing / swipeable / annotation 单测全绿;`tsc`/Biome 通过。

### QA#47(2026-06-15 de889f1 prod build,tab-1 vs tab-2):P4 后 burst 收敛(cadence < ~600ms)

commit `de889f1` 桌面 prod build,两 tab 各 6 次切歌,cadence 可比(tab-1 ~489ms、tab-2 ~437ms)。trace 见 [`.logs/commit-de889f10.../`](../../../.logs/commit-de889f10d37645a4a6db5d6c4fbc1cf11955362d/)。

| 指标 | tab-1(Now Playing) | tab-2(队列/库) | afff320(P4 前)tab-1 |
|---|---|---|---|
| `fpsAvg` | **108** | 111.9 / 109.6 / 109.1 | ~90 |
| `fpsLow` | **24** | 13.4 | 7.5 |
| `frameMaxMs` | **41.7** | 74.9 | 133 |
| `heapMb` | 359 | 270–328 | 369 |

**结论:**

1. **✅ tab-1 已与 tab-2 收敛 —— 且 `frameMax`(41.7 vs 74.9)/`fpsLow`(24 vs 13.4)反而更优。**相较 P4 前(afff320 burst ~90 / `frameMax 133` / `fpsLow 7.5`)是大幅改善。
2. **主驱动是 P2 coverflow-skip 这次真正触发。**本轮切歌间隔 **566 / 642 / 542 / 642 / 541ms**(均值 ~587,多数 < `COVERFLOW_BURST_SKIP_MS=600`)→ coverflow overlay 多被跳过、3D 模糊背光不合成 → `frameMax 133→41.7`。P4 memo 减少父级级联重渲染为辅助贡献。
3. **`cover.render` 仍 42 条(~7/切,全 cache-hit)未降,但无所谓** —— 印证 QA#46 的判断:cache-hit 廉价,不是瓶颈;memo 没减少 `SwipeableMediaStage` 自身随 currentIndex 的重渲染(它必须重渲染来驱动切歌),但 fps 不受影响。
4. **⚠️ 已知剩余边界:cadence > ~600ms 的「中速点击」(如 QA#46 的 711ms)仍会每切播放完整 coverflow → `frameMax` 尖峰。**狂按(< 600ms)已解决;中速点击区间未覆盖,取决于 `COVERFLOW_BURST_SKIP_MS` 阈值或 coverflow 自身合成成本。

**仅 1 条 tab-1 fps window 落在 burst 中段(数据偏薄但无歧义,且与 cadence/coverflow-skip 因果自洽)。**
...
### Phase 32-A: AutoScrollText 只在内容/尺寸变化时重测(✅ 代码完成)

**Goal:** 消除 QA#49 定位的 ~1.4s 强制回流风暴(每次切歌每个 AutoScrollText 都 clone→appendChild→reflow→remove)。

**改动([`auto-scroll-text.tsx`](../../../src/components/ui/auto-scroll-text.tsx)):**
- [x] `useLayoutEffect` 从**无依赖(每 render 重测 + 重建 ResizeObserver)**改为 **`[]` 只 setup 一次**;`measure()` 在 mount 跑一次,之后由 observer 驱动。
- [x] `ResizeObserver` 捕获 pill/窗口 resize(只建一次,不再每 render 重建);新增 **`MutationObserver`**(`characterData`/`childList`/`subtree`)捕获**文本变化**——因为静态 `truncate` 盒的*渲染*宽度在换歌时不变,RO 单独会漏(这正是原来用「每 render 重测」兜底的原因,现在改为精确触发)。
- [x] **保留** `measureNaturalWidth` 的 clone-to-body(它是 natural-width 测量的关键、且被单测覆盖),只是从「每 render」降到「每次真实变化」。clone(coverflow 过场卡跳过测量)留待后续,本次频率修复已覆盖主成本。
- [x] 单测:新增「同文本 re-render 不重测」(`auto-scroll-text.test.tsx`,6 例绿;旧 5 例回归 + swipeable/ui 共 35 例绿);`tsc`/Biome 通过。

**预期:**切歌时 AutoScrollText 的 measure 从「实例数 × 每首多次 render」降到「实例数 × 每首 ~1 次文本变化」,削掉 `get scrollWidth`/`remove`/`appendChild`/`measure` 那 ~1.4s 的大部分。待 QA clean prod 复测(Phase 32-B)确认 `frameMax`/heap churn 下降。
...
| 27 | QA#47:cadence > ~600ms 的「中速点击」仍每切播完整 coverflow → `frameMax` 单帧尖峰,要不要覆盖? | 🔲 待拍板 | 三选项:**(a)** 接受现状——狂按(< 600ms)已收敛,中速点击只是偶发单帧 hitch,非持续掉帧;**(b)** 调高 `COVERFLOW_BURST_SKIP_MS`(如 600→900)让中速点击也跳过 coverflow,代价是 coverflow 动画只在 > 900ms 的孤立切歌才播(观感变化);**(c)** P5 直接降 coverflow 单次合成成本(3D `blur(20px) saturate(400%)` 背光预算/降级),让它即便每切都播也不尖峰。 |
...
| 2026-06-15 | User+Claude | **Phase 32-A 代码完成:AutoScrollText 只在内容/尺寸变化时重测**。`useLayoutEffect` 从无依赖(每 render 重测 + 重建 RO)改为 `[]` setup-once + `ResizeObserver`(resize)+ `MutationObserver`(文本变化,catch 换歌——truncate 盒渲染宽度不变 RO 会漏)。保留 clone-to-body(natural-width 关键 + 被测)但频率从「每 render」降到「每次真实变化」。新增「同文本 re-render 不重测」单测;auto-scroll-text 6 + swipeable/ui 35 例绿,`tsc`/Biome 通过。预期削掉 QA#49 的 ~1.4s 回流大部分,待 clean prod 复测(32-B)。 |
...
| 2026-06-15 | Claude | **QA#50 + Phase 32-B carve-out**:HEAD(80e0483,含 32-A)新 tab-1 切歌 trace 确认限速/音频 abort/Pixi 192px 派生/preload 收敛/AutoScrollText 频率修复**均生效**,但 worst-case 仍 `fpsLow 4.8`/`frameMax 208`/`longTask 203`/heap 140→382→GC(7 切 cadence ~270ms)。`blobsCreated(image)` 仅 +2、`cover.preload created:0`、`cover.render` 全 cache-hit → **排除整图解码**;残留 = ~35MB/切 非-blob 分配 + 仪表污染(本日志仍开 RDT/HUD/逐首 DEBUG → 不能用它验收 32-A)。按 prd-create §4「观测先行」把 QA#48 的深层 GC 残留 carve 成焦点文档 [now-playing-switch-gc-closure PRD](../20260615-muzero-now-playing-switch-gc-closure-prd/20260615-muzero-now-playing-switch-gc-closure-prd.md)(Phase 1 clean-prod 复测 + DevTools Allocation 归因 → Phase 2 最小收尾)。32-B 起追踪移交该 PRD。 |
