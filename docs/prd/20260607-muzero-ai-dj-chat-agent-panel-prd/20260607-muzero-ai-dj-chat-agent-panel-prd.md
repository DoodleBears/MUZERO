...
|---|---|---|---|
| `library_search_tracks` | `muzero.library.search_tracks` | 读 | [`track-search.ts`](../../../src/lib/track-search.ts)（搜 tags + 各曲 memory.note）over `listAllTracks`。**多关键词** `queries[]`（per-term 搜、`match` any=并集/all=交集），**字段投影** `fields`（默认仅 `id`+`title`，缩小 JSON 回传），**`limit` ≤500 + 游标翻页** `cursor`（回 `{total, returned, nextCursor, tracks}`，`total` 是完整命中数、`nextCursor` 非 null 时把它当 `cursor` 续翻直到 null）|
| `lyrics_search` | `muzero.lyrics.search` | 读 | **【新】按歌词找歌**（"那首唱…的歌"）——`executeLyricsSearch`：`queries[]`+match any/all over `lyricsSearchFields`（存储 `db.lyrics` 行的 plain/synced/translation/romanization **+ 生成曲 brief.lyrics**，时间戳/逐字标签忽略），回 trackId+title+**命中歌词片段**，cursor 翻页。`library_search_tracks` **不搜歌词**（轻量），歌词搜走这个 |
