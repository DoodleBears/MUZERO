...
**Status:** Implemented（Phase 1–6 全 ✅ 落地；外壳已重设计为 dock 集成三态入口并挂进 player-dock；多 provider + 动态自定义 provider + per-session 模型选择 + auto-dispatch + 空态 onboarding + 上下文预算拦截全接线 + i18n ×4。2026-06-13 compact activity popover 已落地并通过组件测/类型/lint；Browser 视觉自动化受当前 Windows sandbox 阻塞，需真实窗口人工复核。余：Phase 3 store-pump E2E 仍 blocked 于 player-store 并行 WIP；真实 LLM/endpoint 端到端对话待人工。分支 `feature/ai-dj-local-id-tree-activity`）
**Created:** 2026-06-07
**Author:** MUZERO
...
| 2.1 | Compact Agent Activity Popover（icon/chip 态 tool-call 实时状态，2-line 文本自动滚动无 scrollbar） | ✅ Completed（2026-06-13：`chat-activity-popover` + `dj-chat-entry` compact 接线 + top notification 协调；Browser 视觉自动化 blocked，见 checklist） | §5.2.1 / §7 |
...
- **Agent**：`new ToolLoopAgent({ model: await resolveDjModel(settings), tools, stopWhen: stepCountIs(12), instructions: \`${DJ_CHAT_SYSTEM_PROMPT}\n\n${await buildNowPlayingContext(db)}\`, temperature, maxOutputTokens })`（now-playing 快照每轮重建，见 §4.2 约定）。
...
|---|---|---|---|
| `library_search` | `muzero.library.search` | 读 | **统一搜索（复用 ⌘F 引擎），按 `types` 过滤类型**（默认 `["track"]`）——`executeLibrarySearch`：`queries[]`+`match` any/all，**只回所请求的组**。`"track"`→[`track-search.ts`](../../../src/lib/track-search.ts) `searchTracks`/`scoreRow`（含 memory.note，**字段投影** `fields` 默认 id+title、`cursor`/`nextCursor` 翻页）；`"set"`→`freeTextMatches` 搜歌单**名**（{id,name,trackCount}）；`"lyrics"`→`findLyricSearchMatch`（与 ⌘F 同款，含存储歌词 + brief.lyrics，时间戳/逐字忽略）回 trackId+title+**命中行片段+timeSec**。三组全用 search-core 的**音译感知**匹配（拼音/假名/罗马音）。合并了原 `library_search_tracks`+`lyrics_search` |
...
| `set_create` | `muzero.set.create` | 写·免费 | `createSession`（seed/config）**+ 可选 `trackIds[]` 一步建单并灌入**（`executeCreateSet`：建集→`prependTrackIds` 保序、跳过未知 id，回 `{…session, addedTrackCount}`）——「用这几首建个歌单」一次调用搞定，不用 create 再 add |
| `set_update` | `muzero.set.update` | 写·免费 | 改名/config |
| `set_add_tracks` | `muzero.set.add_tracks` | 写·免费 | **【新】把现有本地 track id 加进歌单（策展核心）**——「搜出所有 lofi → 建新歌单」。`prependTrackIds`（幂等，只加存在的本地曲、跳过已在的）。**目标可是已有歌单**（`set_list`/`set_get` 拿 id）**或新建的**——不止建新单。用于**手挑少量** id |
| `set_add_by_search` | `muzero.set.add_by_search` | 写·免费 | **【新】一步策展**：按 `queries[]`（match any/all）搜**全库**、把每条命中**直接灌进歌单**——agent 无需把每个 id 列进上下文（大库不爆 token）。回 `{matched, added, skipped}`。**目标可是已有歌单（扩充）或新建**。用于「把整个流派/心情做成/扩进歌单」 |
...
| `memory_search` | `muzero.memory.search` | 读 | **【新】按关键词搜记忆**（`queries[]` + match any/all）——`executeMemorySearch` over `listAllMemories`，匹配 memory.note **+ 该曲 title/tags**，每条命中**带 trackId+title**（可顺手 play/curate 那首）。回 `{total, returned, memories[]}` |
| `add_memory` | `muzero.memory.add` | 写·免费 | `executeAddMemory`：给指定 track 记一条 note；**`trackId` 可省 → 落到「正在播放的曲」**（`getPlayQueue` 取当前），让你**听歌时对话加记忆**（"给这首记一句『写代码神器』"）。空队列且无 id → 报错。一曲多条（见数据模型 PRD）|
...
- **now-playing 每轮注入 system（已实现 CHAT-5n）**：`buildNowPlayingContext(db)`（[`dj-chat-context.ts`](../../../src/chat/dj-chat-context.ts)）每轮读 `getPlayQueue` → 当前曲（**title + id**）+ 活跃歌单（**name + id + 曲数**）+ 队列位置（position N of M），append 到 `instructions`，让「这首」「这个歌单」有所指、可直接拿 id 操作（加进活跃歌单 / 切歌 / 续 vibe），不必先花一次 `now_playing_get`。空队列安全。后续可扩 tags/记忆摘要/upcoming。`now_playing_get` 仍作补充读工具。
...
- 点 chip 右侧「展开」钮 / 点回复通知 → `mode="expanded"`：从 dock 入口处**framer-motion 过渡**成一个对话 widget，承载完整 panel body（header + `chat-turns` 历史 + `chat-composer` + session 控件 + 队列托盘）。**header 不放 per-session 模型 picker**（owner：expand 顶部切模型「不需要」）——模型只在 Settings 全局配置（用 `ModelCatalogCombobox`）。移除后 `ChatModelPicker` 组件**已无挂载点**（孤儿，留作可复用 primitive 或后续删除）。**实现注记**：icon↔chip 间用 `layoutId` 共享布局 morph；chip→widget 用 widget 自身 spring（origin-bottom scale+y+fade，从 dock 区域长出）——widget 经 portal 渲染（dock 容器带 centering transform，`fixed` 相对它解析），跨 portal 共享 `layoutId` 会死锁 motion projection。
...
- v1 用**新 `textarea` 原语**；**autosize：默认 1 行行高，随换行/折行长高到 3 行（`MAX_ROWS`），超出则内部滚动**（`useLayoutEffect` 先把 `height` 置 `auto` 再设 `min(scrollHeight, 3 行+padding)`，并据此切 `overflow-y`）。`textarea` 原语补 ref 透传（React 19 ref-as-prop）。@mention（track/session 引用）做成**后续增强**（ClipCombo 用 contentEditable+chips，较重，先不抄）；`/` slash 命令已落地（见下）。
...
- **`/` slash commands**：草稿是单个 `/token`（还没空格）且有命中时，composer 上方浮**命令菜单**（`role="listbox"`/`option`，↑↓ 高亮、`Enter`/点选运行、`Esc` 收起到下个键入）。命令由宿主注入（`ChatComposer` 的 `slashCommands` prop → `ChatPanel` → dj-chat-entry）：当前 `**/new**`（开新 session）+ `**/history**`（开历史），可扩展。无命中则照常当普通消息发送。@mention（track/session 引用）仍是后续增强。
...
- 每 turn：user 气泡 sticky 顶部；assistant parts **按实际输出顺序交错渲染**（`renderMessageBlocks`：遍历 `message.parts`，连续 text 合并成一个 streamdown 块、遇到 tool 先 flush 文本再渲 collapsible）——多步交互「text→call→text→call」如实呈现，**不再把所有文本堆在工具调用上方**（CHAT-5o）；per-turn **edit-and-resend**（= regenerate，复用 `messageId` 让 SDK 截断重流）；成本/token footer。
...
| Provider 选择 = Popover+Command **带品牌 icon**；「+ Add Custom Provider」建 `custom:${uuid}` | **`provider-combobox.tsx`**（每 item 品牌 glyph + label + key 状态、trigger 显示选中 icon，替换 grid）+ `provider-brand-icons.tsx`（CC0 内联 SVG）+ Command 原语加 `icon`/`description` slot；model picker/catalog combobox 同款 icon | ✅ CHAT-5f |
...
| `ApiKeyField`（遮罩 + reveal + `apiKeyUrl` 链接 + onCommit 提交）| 内联于 `llm-provider-settings.tsx`（每 provider 一枚，写 `apiKeysByPresetId[id]`）| ✅ CHAT-5c |
| `createClipChatLanguageModel(settings)`：anthropic vs openai-compatible / keyless-local 去 auth header / baseURL 归一（补 `/v1`）| `resolveDjModel` 扩（[`src/ai/model.ts`](../../../src/ai/model.ts)）：anthropic `createAnthropic`+浏览器直连 header；其余 `createOpenAI({ baseURL, apiKey, fetch })`；keyless-local `fetchWithoutAuthorization` | ✅ CHAT-5b |
| **动态 model 目录（ClipCombo 只做 OpenRouter，MUZERO 通用化）**：拉 provider `/v1/models` 实时列表，combobox 可过滤 + 自由输入自定义 id | **新 `src/ai/model-catalog.ts`**（`modelsEndpointFor`/`parseModelCatalog`[OpenRouter rich + 通用]/`fetchModelCatalog` 缓存+dedup）+ `use-model-catalog` hook + `model-catalog-combobox.tsx`（hardcoded ∪ live ∪ 自定义 id + loading/refresh）；覆盖 OpenRouter/OpenAI/Groq/DeepSeek/Gemini + **本地 ollama/vLLM/LM Studio**；Anthropic 无开放列表→hardcoded | ✅ CHAT-5e |
...
**Tasks:**
- [x] 新增 `chat-activity-popover.tsx` 展示层：接收 `activity` view model + labels，无内置文案。
- [x] 新增纯函数 `deriveChatActivity(snapshot, labels)` 或同等 selector：
...
- [x] `dj-chat-entry.tsx` 在 `mode ∈ {icon, chip}` 且 active session running 时渲染 popover，锚在 entry 上方；`expanded` 时卸载。
- [x] 文本预览最多 2 lines：无 scrollbar；长文本自动纵向滚动，`prefers-reduced-motion` 下改 `line-clamp-2`。
- [x] Popover 点击展开到 `expanded` 并打开 active session；`Esc` / outside dismiss 只关闭 popover，不 stop Agent。
- [x] i18n `chat.activity*` keys ×4（状态动词、aria label、waiting approval、error fallback）。
- [x] 与 `chat-reply-notification` 协调：active compact turn 用 activity popover；后台 session / dismissed completion 才用 top notification。

**Phase 2.1 Checklist:**
- [x] 组件测：icon 模式 streaming tool-call → popover 显示 tool label + loader，点击展开。
- [x] 组件测：chip 模式 assistant text delta 超过 2 行 → 容器高度不超过 2 lines，`scrollbar-width: none` / `overflow` 不显示 scrollbar。
- [x] 组件测：pending approval 不自动消失；`Esc` / outside dismiss 只关闭当前 turn 的 popover，不 stop Agent。
- [x] 组件测：`mode="expanded"` 不渲染 popover。
- [x] reduced-motion 测试：无自动滚动动画，文本 line-clamp。
- [x] Browser 视觉验证尝试：本地 Vite `127.0.0.1:41730` 可响应；in-app Browser 在当前 Windows sandbox 下 `CreateProcessAsUserW failed: 5`，且本地无 Playwright/Puppeteer 依赖，无法自动截图。需在真实前台窗口人工复核 desktop 1180×780 与 mobile 宽度下 popover 不遮挡 PlayerDock controls、不与 memory/nav icon 重叠。
...
| Date | Author | Changes |
|------|--------|---------|
| 2026-06-13 | Codex | 完成 **Phase 2.1 Compact Agent Activity Popover**：新增 `deriveChatActivity` + `chat-activity-popover`，接入 `dj-chat-entry` compact icon/chip 态；tool-call/assistant delta/approval/error/queued 状态统一展示，preview 限 2 行、无 scrollbar、motion-reduce line-clamp；点击展开，Esc/outside 仅 dismiss 当前 turn；`chat-reply-notification` 收窄为完成态提示。验证：3 个 chat 组件测 23 tests、Biome、`tsc --noEmit` 通过；Browser 视觉自动化受 sandbox 阻塞，需人工窗口复核。 |
...
| 2026-06-12 | Claude | **CHAT-5s 统一搜索 + 类型过滤（复用 ⌘F 引擎）**（owner：「⌘F 的歌词搜索其实是一样的，能抽象成一个搜索功能吗？能按不同类型过滤吗」）。把 `library_search_tracks` + `lyrics_search` **合并成一个 `library_search`**，加 `types`（默认 `["track"]`，可含 `"track"`/`"set"`/`"lyrics"`，只回所请求的组）。关键是**复用 ⌘F overlay 的同款引擎**：`"track"`→`searchTracks`/`scoreRow`（音译感知，含 memory.note、fields 投影、cursor 翻页）；`"set"`→`freeTextMatches` 搜歌单名；`"lyrics"`→**`findLyricSearchMatch`**（与 ⌘F 逐行同款，替换之前自研的朴素 `.includes`+snippet）回命中行+timeSec。删 `executeLyricsSearch`/`lyricsSearchInputSchema`（折叠进 `searchLyricHits` 共享 helper）。system prompt 改为「`library_search` 按 types 过滤」。§4.2 两行并一行。`library_search` 33 测全绿（含 default-track-only/types=lyrics 带 snippet/存储歌词忽略时间戳/types=set 名搜/多组同回 + any·all）、tsc 清、biome 清。|
| 2026-06-12 | Claude | **CHAT-5r 按歌词搜索**（owner：「是否支持根据歌词搜索？感觉可以增加一个」）。地基已在（`track-search.ts` 的 `lyricsSearchFields` 解析存储歌词 + brief.lyrics），缺的是 chat 工具——`library_search_tracks` 不搜歌词（轻量、不 join 歌词表）。新增 **`lyrics_search`**（`muzero.lyrics.search`，读·免费）：`queries[]`+match any/all、`limit`+`cursor` 翻页；`executeLyricsSearch` over **新 repo `listAllLyrics`**（一次性 load `db.lyrics` 建 Map，避免 N 次 join）+ `lyricsSearchFields`（plain/synced/translation/romanization 解析 + brief.lyrics，时间戳/逐字标签忽略），回 `{total, returned, nextCursor, tracks[]}`，每条带 trackId+title+**命中歌词片段**（`lyricSnippet` 取首个含关键词的行、剥 `[mm:ss]` 标签、截断）。system prompt 补「按记得的歌词找歌用 lyrics_search」。§4.2 加行。新增 3 测（brief 歌词命中带片段/存储歌词忽略时间戳/any·all·cursor）。chat 套件全绿、tsc 清、biome 清。|
| 2026-06-12 | Claude | **CHAT-5q 记忆搜索 + 记忆落到当前曲**（owner：「能搜索/创建 memory 吗？关键词 array 搜 memory（带 track 信息），再给指定 track 加 memory？」）。澄清:`add_memory` 早已能给指定 track 加记忆，缺的是**搜索记忆**。**① `memory_search`**（`muzero.memory.search`，读·免费）：`queries[]`+match any/all，`executeMemorySearch` over **新 repo `listAllMemories`**，匹配 memory.note **+ 该曲 title/tags**，回 `{total, returned, memories[]}`，每条带 **trackId+trackTitle**（可顺手 play/curate 那首）。**② `add_memory` 增强**：`trackId` 改可选——省略时 `executeAddMemory` 用 `getPlayQueue` 落到**正在播放的曲**（"给这首记一句…"），空队列且无 id 报错；返回 `AgentWriteResult`（带 memoryId/trackId）。system prompt 补记忆段，§4.2 工具表加 memory_search 行 + 改写 add_memory 行。新增 6 测（加记忆/省 id 落当前曲/空队列报错/关键词搜含 track 信息/标题命中/match all）。chat 套件 28 测全绿、tsc 清、biome 清。|
| 2026-06-12 | Claude | **CHAT-5p token 上限可调 + set_create 一步建单灌入**（owner：「有没有限制 input/output token？不要卡死、允许调大」+「创建歌单不太顺畅，create 是不是给歌单信息+trackId array？」）。**① token 上限可调 + 默认调大**：`maxOutputTokens` 1200→**默认 32000**（owner 定）、读 `settings.chatMaxOutputTokens`（避免多步工具/长回复被截断）；上下文预算 maxTokens 仍默认 128k 但 `ChatPanel` 加 `contextMaxTokens` prop、dj-chat-entry 传 `settings.chatMaxContextTokens`（大上下文模型可调高、长对话不早早被 block）。Settings「LLM」面板加「DJ 对话 token 上限」两个数字输入（回复/上下文）+ i18n ×4。**② `set_create` 接 `trackIds[]`**：回应「创建歌单不顺畅」——`set_create` 原本只收 name/seed/autoExtend、建集与加曲是两次调用；现加可选 `trackIds`，`executeCreateSet` 建集后 `prependTrackIds` 保序灌入（跳过未知 id），回 `{…session, addedTrackCount}`，「用这几首建歌单」一次调用完成。system prompt 更新建单流程。新增 3 测（建单灌入保序/跳未知、空建单）。chat 套件全绿、tsc 清、biome 清。|
| 2026-06-12 | Claude | **CHAT-5o 消息按输出顺序交错渲染**（owner：「文本和 tool call 应按实际输出顺序交错，不该总是文本在 tool call 上面」）。`chat-turns.tsx` 原本 filter 出所有 text join 成一块、再把所有 tool 渲在下面 → 多步交互文本永远浮在工具上方。改 `renderMessageBlocks`：遍历 `message.parts` 保序，连续 text 合并成一个 `Streamdown` 块（markdown 跨 delta 仍成整体）、遇到 tool part 先 flush 文本再渲 `ChatToolCollapsible`，末尾再 flush——「text→call→text→call」如实交错。新增交错顺序断言测（`compareDocumentPosition`）。chat-turns 2 测全绿、tsc 清、biome 清。|
| 2026-06-12 | Claude | **CHAT-5n chip slash commands + now-playing 注入 system**（owner：「normal 形态也要能 `/` slash；system prompt 要带当前播放的歌单+歌曲 id/标题」）。**① slash 抽成共享模块** [`slash-commands.tsx`](../../../src/components/chat/slash-commands.tsx)（`useSlashCommands` hook + `SlashMenu`），`ChatComposer` 与 **dock chip**（折叠 normal 形态的内联 `<input>`）都用它——chip 输入 `/` 即在其上方浮命令菜单（同 listbox/option + ↑↓/Enter/Esc）；chip 的 `/history` 先 `setMode("expanded")` 再开历史，从折叠态也有用。`slashCommands` 在 dj-chat-entry 单一定义、chip+ChatPanel 共用。**② now-playing 注入 system 落地**（PRD 早有计划、之前未实现）：`buildNowPlayingContext(db)` 每轮读 `getPlayQueue` → 当前曲 title+id、活跃歌单 name+id+曲数、队列 position N of M，append 到 `ToolLoopAgent.instructions`；system prompt 加一句引导用「Now playing」块直接操作当前上下文。空队列安全。新增 `dj-chat-context.test`（3 例：空/有集有曲/无集有曲）。chat 套件 26 测全绿、tsc 清、biome 清。|
| 2026-06-12 | Claude | **CHAT-5m 去掉 expand 顶部模型选择 + `/` slash commands**（owner）。**① 移除 per-session 模型 picker**：expand widget header 不再渲染 `ChatModelPicker`（owner：「顶部切换模型可以不需要」）——模型只在 Settings 全局配；删 dj-chat-entry 里相关 import/派生（`useCustomLlmProviders`/`llmSelectionForChatSession`/`enabledLlmPresetIds`/`resolveLlmProviderPreset`/`setChatSessionLlm`/sessionSelection/enabledPresets）。移除后 `ChatModelPicker` 成孤儿组件（Settings 全局默认用的是 `ModelCatalogCombobox`，非它），留作可复用 primitive 或后续删除。**② slash commands**：`ChatComposer` 加 `slashCommands` prop + 浮动命令菜单（草稿是单 `/token` 且有命中时显示，`role=listbox`/`option`，↑↓ 高亮 + `Enter`/点选运行 + `Esc` 收起；无命中照常发普通消息）；`ChatPanel` 透传；dj-chat-entry 注入 **`/new`**（开新 session）+ **`/history`**（开历史）。i18n `chat.slashNew`/`slashHistory` ×4。重写 §5.3/§5.4。chat 套件 23 测全绿（+4 slash 测）、tsc 清、biome 清。|
| 2026-06-12 | Claude | **CHAT-5l 游标翻页 + 扩充已有歌单**（owner：「限制单次回传数量就要支持 Cursor 翻页；要支持往已有歌单加曲，不只是建新单」）。**① 游标翻页**：`library_search_tracks` 加 `cursor`（offset，默认 0），回传加 `nextCursor`（`cursor+returned < total` 时为下一页 offset，否则 null）；agent 把 `nextCursor` 当 `cursor` 续翻直到 null——单次回传可放心设上限。**② 已有歌单一等公民**：`set_add_tracks`/`set_add_by_search` 本就接受任意 `sessionId`，但描述/prompt 偏向「建新单」；改写两者描述 + system prompt，明确**先 `set_list`/`set_get` 找到已有歌单 id → `set_add_by_search`/`set_add_tracks` 扩充**，`set_create` 只在要全新歌单时用。新增 2 测（游标翻页不重不漏覆盖全集、`set_add_by_search` 扩充已有成员歌单）+ 既有测补 `nextCursor` 断言。dj-chat-tools 21 测全绿、tsc 清。|
