...
- [ ] `scripts/generate-release-meta.mjs` → `src/generated/release-meta.ts`（`app_version` + `git sha` + `build ts` + `shell`）；接进 `prebuild`/`predev`。
- [ ] `vite.config.ts` 加 `define: { __APP_VERSION__: JSON.stringify(pkg.version) }` + `vite-env.d.ts` 声明。
- [ ] `src/lib/app-version.ts`：`export const APP_VERSION = __APP_VERSION__` + `RELEASE_ID`（唯一读取点，注释「别处不许硬写版本」）。
- [ ] `src/lib/compare-semver.ts` + test（prerelease 排序）。
...
- [ ] `compareSemver` 单测覆盖 release/prerelease/相等
