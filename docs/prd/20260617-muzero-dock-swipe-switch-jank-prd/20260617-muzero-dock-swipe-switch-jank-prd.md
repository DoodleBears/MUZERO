...
| 2 | 收窄 external switch 冷滑成本（overlay 按需 mount / 让相邻 external 走轻量 crossfade） | 🔲 Pending | [Phase 2](#phase-2) |
...
### Phase 2
**Goal:** 压低 external switch 的冷滑切歌→帧阻塞（dock 拖拽 / 上一首下一首按钮 / Q/E / 队列点击 共享受益）。CDP 实拖归因显示：切歌帧的大头是 `setActive(true)` 在切歌帧**新挂载 coverflow overlay**（portal + `CoverPagerStrip` 5 槽）+ `updateOverlayRect` 的 `getBoundingClientRect` 强制 reflow——封面**自身**手势时 overlay 已 active、commit 不重挂，故便宜。
- [ ] 候选 A：相邻 external 切歌**跳过 overlay 冷滑**，直接走 base `MediaStage` 轻量 crossfade（少一次大 DOM 挂载）；远跳已是 snap。需权衡「transport 按钮切歌还要不要 coverflow 滑动感」。
- [ ] 候选 B：external switch 的 overlay setup（rect 测量 / setActive / 窗口重算）移出切歌**首帧**（rAF 切片），降 `switchToFrame`/`frameMax`。
- [ ] 用 `perf-gesture dock` before/after 验证；目标 Dock `switchToFrame` 向封面（123ms）靠拢。
