...
| 2026-06-14 | Claude | QA 修(字母条拖拽错位:按 C 拖到 W 松手仍跳 C):根因——每个字母是带 `onClick` 的 `<button>`,pointer capture 落在**按下的那个**字母上,松手合成的 trailing click 打在 C 上 → `onClick(C)` 覆盖了拖拽过程 `onPointerMove→jumpAt(W)`。修法:字母改 `pointer-events-none` 的纯 `<span>`(去 `onClick`),所有跳转交给 strip 的 pointer 事件;`setPointerCapture` 改在 strip(`currentTarget`)而非按下的字母。一处修四个 rail。library 全绿 |
| 2026-06-14 | Claude | Rollout 收尾:**歌单/专辑/歌手详情页的歌曲列表**也接上 A–Z(它们本就用 `TrackListSection`→`VirtualTrackList`,滚动条+rAF+惰性+memo 已有)。抽共享 hook [`useTrackAlphabetLetterOf`](../../../src/components/library/use-track-alphabet.ts)(名称排序 + 无 query + 非红心 + `>30`),全部歌曲/`EntityDetailView`/`SetDetailView` 三处复用。pages+library 全绿 |
