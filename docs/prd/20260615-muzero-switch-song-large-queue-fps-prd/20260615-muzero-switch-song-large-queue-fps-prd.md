...
| 2026-06-15 | User + Claude | **根因修复：playCount 去 `tracks` 行非规范化（120→20 雪崩,待 QA）**。trace 实锤(`listen.flush counts=true` → `listAllTracks`/`trackPlaybackStats` requery → `queue.live.fetch 451ms changed:false`):**听够阈值切歌时 `recordPlaybackListen` 给 `tracks` 行写 `playCount+1`** → 毒化**所有 `tracks` liveQuery 观察者**:Phase 2 队列 sub(getTracksByIds 5983 = 451ms 白重取)+ search-page 后台 tab 的全表 `listAllTracks` → O(5983) 派生级联(memoryNotes+worker 搜索+索引)持续数秒 = 120→20 雪崩。修:**`recordPlaybackListen` 不再写 `tracks.playCount`**(权威计数本就在 `trackPlaybackStats`);唯一读者 DJ `projectTrack` 改从 `trackPlaybackStats` join(`sumPlayCountsByTrack`,仅当 agent 请求 playCount 字段、仅当前页)。`reconcileTrackPlayCountsFromStats` 仅测试调用、`incrementPlayCount` 无调用方——`Track.playCount` 生产态已无写者。chat 108 + stats/playlists/player-store 49 例全绿。**遗留**:其它 `tracks` 写(改封面/like/tags)仍会触发 search-page 后台全表派生 → best-practice #7「tab 不可见时暂停 search-page liveQuery」作为防御性后续。liveQuery best-practice 见分析。 |
| 2026-06-15 | User + Claude | **封面裁剪风暴修复（方案 A，待 QA）**：用户拍板"直接用原图"。在**三处 `getCroppedBlob` 调用点**(显示 `use-media.useTrackCoverResource`、预载 `cover-preload`、`playback-preload.warmTrackCover`)统一**去裁剪**——封面一律渲染原图 blob，cache key 一律 = `coverBlobId`(三处一致,命中率更高),`<img>` 的 `object-fit:cover` 负责视觉适配。**消除主线程 canvas 裁剪**(`cover.crop.canvas`)及其重渲染风暴。`coverCrop` 数据保留在 DB(未删),per-cover 裁剪定位功能丢弃。`coverCropped` 设置 + 裁剪 UI 现为 no-op,待与 album-cover-appearance 协调清理。typecheck + cover-preload/cover-performance 6 例绿。 |
| 2026-06-15 | User + Claude | **Phase 2 QA 通过 ✅（图1）**：连切时 `queue.live.fetch` **count 恒为 ×1**（不再随每切增长），fps 84，游标写不再 re-fire 全量重取——拆订阅生效。**新发现（图2，独立严重）**：**设置/改封面后掉到 16 fps**——`cover.render.object-url-miss` ×31@1632ms + `cover.crop.canvas` ×7@1626ms + `cover.preload.batch` ×43@1903ms。根因 = `getCroppedBlob`（[image-crop.ts:19](../../../src/lib/image-crop.ts#L19)）在**主线程 canvas** 上 load-image→drawImage→`toBlob` 重编码，封面改动使 cover cache key 失效 → 各 surface **重解码+重裁+重编码全分辨率封面**。**非 Phase 2 回归**（`queue.live.fetch` 仅 ×2）。修向：crop 移 off-main（OffscreenCanvas/worker 或 createImageBitmap）+ 裁剪结果缓存/持久化。与 album-cover-appearance 工作重叠，待协调归属（可能 Phase 4 或并入那条线）。 |
