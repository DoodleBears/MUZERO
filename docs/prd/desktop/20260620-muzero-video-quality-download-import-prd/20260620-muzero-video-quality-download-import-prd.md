...
- [x] **封面（cover）**：下载入库时自动配封面（best-effort，不阻塞下载）——优先源官方封面 URL（B站 `pic` / YT thumbnail，下载到本地 cover blob、离线 + WebGL 安全），无则从下载的视频用 [`extractUsefulVideoPosterFrame`](../../../../src/lib/video-poster-frame.ts)（mediabunny/native）**抽帧**作封面。E2E：YT `EvuXIk2Bh78` 显式 id（无源封面）→ 抽帧 → `hasCover:true`（+ 生成 backlight 派生）。✅
