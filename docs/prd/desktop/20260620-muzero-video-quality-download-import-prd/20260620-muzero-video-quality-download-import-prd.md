...
- [x] **下载入库 + 离线播放 实时 E2E（harness，task #9）**：`downloadStreamedVideoToLibrary`（[`download-to-library.ts`](../../../../src/streamsrc/download-to-library.ts)）resolve→fetch/blob→mux→`cacheStreamedTrackBlob` 落库。登录后 `BV1X163BQEo8` @1080p → 建 **kind:video / origin:streamed / blobId 已填 / downloaded* = 1080·mp4·avc+aac** 的 track 进「Downloads」集；经控制端点 `setActiveSession`+`playIndex` 驱动，`/state` 回 **isPlaying=true、displayMode=video**，媒体 blob 落 `persistent-media/media/…blb_….mp4`（110MB）→ **app 内离线播放成立**。4 单测 + E2E。✅
- [x] **粘贴链接 / 裸 id → 定向解析（不走关键词搜索）**：[`stream-link.ts`](../../../../src/streamsrc/stream-link.ts) `parseStreamLink` 加 Bilibili(`/video/BV…|av…`，忽略 `?spm_id_from`/`vd_source` 等参数)+ YouTube(`watch?v=`/`youtu.be`/`shorts`/`embed`/`playlist?list`)；新增 `parseBareStreamId`（裸 `BV…`/`av…` 与 11 字符 YT id；BV 是 12 字符不冲突）。⌘F hook（[`use-online-source-search.ts`](../../../../src/hooks/use-online-source-search.ts)）`parseStreamLink(q) ?? parseBareStreamId(q)` → 命中即 `getTracksByIds` 定向解析、**跳过关键词搜索**；普通文字 → null → 正常搜索。E2E（6 例）：`BV1HLz9BJEgi` 全 URL/带参 URL/裸号、YT `shorts/0EbmNplrNqE`/裸 id 均解析出正确标题+作者+官方封面；`"周杰伦 七里香"` → `ref:null`（仍走搜索）。✅ 32 单测。：下载入库自动配封面(best-effort，不阻塞下载)——**优先源官方封面**：Bili `view.pic` / YouTube `getBasicInfo` thumbnail，经各源新增的 `getTracksByIds`（[`bili-source`](../../../../src/streamsrc/bili/bili-source.ts) view API · [`youtube-source`](../../../../src/streamsrc/youtube/youtube-source.ts)+`youtube-ytjs` `resolveMeta`）取得 → 下载到本地 cover blob(离线 + WebGL 安全)；无官方封面才从视频 [`extractUsefulVideoPosterFrame`](../../../../src/lib/video-poster-frame.ts) **抽帧**兜底。另有 `recoverStreamedTrackCover` 为**已有 track 省流补封面**(只取封面图、不重下视频)。E2E:Bili 1080p track `via:"official"`(hdslb)、YouTube track `via:"official"`(i.ytimg maxresdefault) 均补上官方封面;6 单测。✅
...
| 2026-06-20 | DoodleBear | 默认视频清晰度设置:`AppSettings.defaultVideoQuality`(Settings→在线源,Auto/2160/…/360);下载未显式选档时(批量 / 「下载全部分P」)用它,源 selector **优先匹配否则降级**。E2E:请求 2160(不可用)→ 降级下 1080;默认 480 → 无显式选档下到 480 |
