...
| 2026-07-03 | DoodleBear | **Post-ship 修正（进度条「跳」而非分批滚动）**：PM 反馈左上角进度不像 batch 推进。根因——Phase 2 把写库改 O(n) 瞬时后，进度却仍**只接在写库**（`addHitsToSet` 的 per-hit `onProgress`）：该循环无 `await`、n 次 `notify.update` 同步打完 → React 合并成一帧直接满格；而真正耗时、天然分批的 **fetch 段**（网易云 `songDetailHits` 按 500 一批拉 `song/detail`）零进度上报，bar 在 `progress===undefined` 时甚至不渲染。**Phase 2 优化反而暴露了这个错位**（优化前 O(n²) 写库慢，per-hit 进度尚能肉眼爬）。修法：把进度信号从写库移到 fetch——`importPlaylist` opts 加 `onProgress(done, total?)`；netease 每批报 `(已处理 id, 总 id)`（总数开头已知→确定性 bar，配合通知条既有 `transition-[width] duration-200` 平滑滚动）；bili 分页报递增计数（总数未知→仅计数无 bar）；`fetchPlaylistHits`/`importStreamedPlaylist`/`addStreamedPlaylistToSet` 转发到 fetch、写库段不再喂进度（避免 fetch 满格后回跳）。新增 netease 回归测试（1200 首→3 批 `(500/1000/1200, 1200)`）。`tsc` 绿、biome 6 文件 0 违规、streamsrc+player-store+import-notification 468 例回归绿。零 schema、零新基建。 |
