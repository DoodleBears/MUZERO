...
> **#3 新发现——背景闪黑（#1 修复后暴露的既有缺陷，harness 实证定位）**：用户复测「标题不闪了，但整个模糊背景（满屏 Pixi）commit 后过渡到黑再 fade in」。这是 #1 修好标题后**暴露的既有 bug**（此前标题闪烁与之同时发生、掩盖了它）。
> - **两次尝试**：(a39b351) 先把 `isRemoteOrStaleCover` 收窄到仅 remote/streamed，让本地封面恒持可见——对本地封面正确，但**用户复测仍闪黑**。
> - **harness 实证**（CDP 真拖拽 + commit 后 1.7s 截图突发 + `[bgflash]` 控制台埋点，临时脚本已删）：黑帧 JPEG ≈130KB vs 图像帧 ≈450KB，无歧义。日志显示用户的曲目是 **streamed**（`isRemoteStale:true`），黑窗口（释放后 ~440–760ms）内 `effectiveRenderPixiTarget` 为 null（新 streamed 封面经 muzfetch 代理解析 ~300ms）**且** `pixiHoldsCover` 为 false（streamed）→ className 落 `opacity-0`。**关键**：此刻 lockstep cover window **仍 active**（`windowActive:true`，`clearWindow` 还没跑），Pixi 控制器**正往同一张 canvas 画着已提交封面**——CSS opacity-0 把"有内容的 canvas"整个隐藏了 = 满屏黑。
> - **正解（a76d30c）**：在 [`now-playing-background.tsx`](../../../src/components/player/now-playing-background.tsx) 用 `useSyncExternalStore` 订阅 cover-window store 的 `active`，把 Pixi 图层 className 加上 `|| coverWindowActive` —— 只要 window 还占着 canvas 就保持 `opacity-100`，与 resting 封面解析状态解耦。**post-fix 突发截图：1.7s 内 15 帧全是满清晰图（无黑帧）**。两处改动互补（a39b351 管本地 resting，a76d30c 管 window 期）。
