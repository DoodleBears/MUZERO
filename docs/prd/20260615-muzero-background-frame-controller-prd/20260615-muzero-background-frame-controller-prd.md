...
| Date | Author | Changes |
|------|--------|---------|
| 2026-06-15 | User+Claude | **QA 修复（slice 2，二修）：松手仍「瞬切」。** 真因不是时长调参,是**单位错**:auto-complete 的 `fromProgress` 用了 `info.offset.x`(原始指针位移),但封面实际位置由**受约束的 `x` motion value** 驱动(`dragConstraints 0` + `dragElastic 0.24` → 指针越拖越远而 `x` 几乎不动)。于是 offset.x 算出的 progress 常 ≈1 → remaining ≈0 → ~0ms → 瞬切。改:auto-complete 距离改用 `x.get()`(卡片 + 背景共同骑乘的真实视觉位置,与 slice-1 背景 `transitionProgress` 同源),commit **决策**仍用 offset.x(手指拖了多远)。stage 10 + driver 9 例绿。待复 QA。 |
| 2026-06-15 | User+Claude | **QA 修复（slice 2）：过阈值松手「直接闪现」非顺滑补完。** 根因:auto-complete 用了 velocity-aware `remainingDurationMs`,松手时仍在移动 → `info.velocity.x` 大 → 时长被压到 ~162ms floor,整段剩余距离一帧闪过。改:**去掉速度缩放,只按剩余距离比例**(`COMMIT_DURATION_SEC × remaining`)—— 刚过阈值松手 ~0.7s 顺滑滑行、接近终点松手快速收尾,永不塌到 floor。fling 仍只影响**是否** commit(`shouldCommitRelease`),不再影响落位速度。stage 10 + driver 9 例绿。待用户复 QA。 |
