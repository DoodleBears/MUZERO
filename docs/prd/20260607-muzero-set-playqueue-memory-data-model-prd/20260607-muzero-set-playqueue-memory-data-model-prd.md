...
- [x] `MemoryNotesWaterfall` 展示层测试：新到旧排序、Pretext masonry top-align、照片 alt、完整显示不裁切、edit/delete 回调、空态。
...
| 2026-06-08 | Codex | 打磨 Memory 照片显示：便签内照片改为 `object-contain`，在固定图片区尽量完整显示、不裁切；补 waterfall 图片显示测试。目标测试通过（2 files / 9 tests）；全局 gate 被并行未提交 visualizer WIP 阻塞（lint 格式化 + typed i18n key typecheck）。 |
