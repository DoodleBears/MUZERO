...
| 2026-06-14 | User+Claude | 默认值校正:线上代码一度漂成「全平台默认 off」(与初稿「非 macOS 默认开」相悖)。用户实测 Lenis 让大库列表**更顺**(rAF 合并 vs 原生每事件重算),要求 **Windows 默认开 + 比较「跟手」**。`resolveSmoothScroll` env 加 `isWindows`:`smoothScroll ?? isWindows`(显式存值仍优先、持久);lerp 默认平台化——Windows `WINDOWS_LERP_DEFAULT=0.18`(snappier),其余 `0.10`;Settings 滑杆/开关/Reset 均读 resolved 值,故新默认正确显示、不丢。Mac/Linux 维持 opt-in(用户只点名 Windows)。`isWindows()` 进 `shortcuts.ts`;`resolve.test` 更新真值表 |
| 2026-06-14 | User+Claude | QA 修(Now Playing 左侧封面列在 smooth 模式下无法往下滚):该列内容(封面图异步加载 / 歌词 / DJ 控制台 / 注释 / ListeningNow)在 Lenis attach **之后**才长高,但 Lenis 的 autoResize 只盯 wrapper 自身的 box(flex/grid 列尺寸固定),不重测 `scrollHeight` → 滚动上限**卡在旧的、过矮的值**,滚不到底。修法(通用,`useSmoothScroll` 内一处修所有列):attach 后用 **ResizeObserver 盯 wrapper 的子节点** + **MutationObserver 盯增删子**,变化即 `lenis.resize()` 重测(`resize` 是廉价读、observer 自带批处理)。新增回归单测(attach 即测一次;后续加子节点再测一次)。smooth-scroll 26→27 测全绿 |
