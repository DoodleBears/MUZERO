...
| 2026-06-11 | DoodleBear | **第二歌词源：网易云音乐**（兑现 Q6「后续加源只实现 `LyricsProvider` + 注册」）。新 `createNeteaseLyricsProvider`(eapi `/song/lyric/v1`，复用 streaming 的 `eapiEncrypt` + `StreamHttp` + cloudsearch)；纯映射 `netease-lyric-map`(LRC/纯音乐识别)。**流式网易云曲零配置走精确路径**：`LyricsQuery.neteaseSongId` 来自 `streamExternalId` → 直接按 songId 取官方词，无模糊搜索；`resolveLyricsProviderForTrack` 对网易云 streamed track 永远选网易云(无视全局设定)。其余曲目用 `AppSettings.lyricsProviderId`(Settings 下拉，默认仍 LRCLIB，桌面端可选网易云)。`LyricsProviderId`/`LyricsSource` 加 `"netease"`(R2 manifest enum 同步加)；`auto-fetch` 用 `provider.id` 标 source。新增 `netease-lyric-map`×7 + `netease-lyrics-provider`×5 + `build-query`×3 单测，全绿 |
