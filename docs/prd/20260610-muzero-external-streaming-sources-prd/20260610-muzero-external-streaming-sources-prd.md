...
**🎉 端到端打通（2026-06-10 Electron 手测）：B站搜索+播放+声音+封面背景全通；网易云搜索+播放+声音全通（免费歌）。** 网易云 resolve 诊断确认：部分歌 `data:[{url:null, br:0, freeTrialPrivilege:{…}}]` = **VIP/付费墙**，匿名只能 30s 试听——这是版权墙不是 bug，**整条管线（加密→签名→eapi 端点→CORS/Referer 注入→crossOrigin→播放）成立**。VIP/高音质需**登录**（带 `MUSIC_U` cookie 的会话）= 收尾后的可选增强。**UX 修**：parser 把 `freeTrialPrivilege`(fee 0 也算) → no-permission `vip`；player-store 对 requires-login / vip 显示「需登录该来源（VIP/会员）」toast（i18n×4）而非泛化「播放出错」。

**2026-06-10 Electron 手测发现 + 修复：**
- **网易云搜不到结果** → 两段根因：①（代理）`stream-http` 把 `Referer`/`UA` 别名成 `x-muzero-h-*`，`electron/fetch-proxy.cjs` **没还原** → 已修（代理加别名还原；B站可播已证明 net.fetch 能注入 header）。② 修①后诊断日志显示 `status:200, code:50000005, count:0` → 网易**对 web 的 weapi cloudsearch 端点加了匿名反爬门**（`50000005`）。**修**：搜索改走 **eapi 端点** `/eapi/cloudsearch/pc`（apiPath `/api/cloudsearch/pc`），eapi 路径无此门、且复用已测的 eapi 加密（与 resolve 同款）。5 测无回归，HMR 生效。
- **B站封面图裂开 / Now Playing 背景黑屏** → hdslb 防盗链拦外域 Referer，导致**所有** streamed 封面 `<img>`/`Image()` 403：搜索缩略图、`CoverImage`（舞台专辑图）、`now-playing-background`（模糊背景，画布 drawImage 显示用、taint 不影响纯显示）、track-row、inspector 等。**修（catch-all）**：`index.html` 加 `<meta name="referrer" content="no-referrer">` —— 一处让全 app 的第三方封面加载成功（app 自身 API/媒体走 muzfetch 在主进程注入 Referer，不受影响，安全）；外加 `CoverImage` 两处 img（preload + 渲染）显式 no-referrer 兜底。`resolveStageContent` 对纯音频 streamed track 本就回退 cover（kind≠video），所以舞台显示封面而非黑视频是对的。⚠️ index.html 改动需 **Cmd+R 整页刷新** 生效。
- **Now Playing 背景（WebGL）仍黑**（封面 `<img>` 好了但背景黑）→ 根因更深：默认背景渲染器是 **Pixi/WebGL（noise）**，**跨域封面没 `crossOrigin`+`ACAO` 不能当 WebGL 纹理（taint）**，no-referrer 只救了纯 `<img>` 显示、救不了 WebGL 纹理。**修（三处协同）**：① `useTrackCoverUrl` 把外部封面也走 `mediaProxyUrl` → muzfetch（响应带 `ACAO:*`）；② `fetch-proxy.cjs` media form **删掉元素自带的 `Referer/Origin`**（localhost 会被 hdslb 403，只用注入的 `__mzh_*`；封面不注入=无 referer，hdslb 放行）；③ Pixi `loadImage` 对 muzfetch URL 设 `crossOrigin="anonymous"` → 纹理 CORS-clean。⚠️ fetch-proxy 改动是主进程 → **需重启 Electron**。
- **标题/歌手/专辑不显示**（列表显示「标题/标题」）→ **用真实 eapi 请求核实**：搜索回传字段齐全（`name`/`ar:[{name}]`/`al:{name,picUrl}`/`dt`），hits 是对的。根因在**显示层**：`trackSubtitle`/`trackArtists` 读 `mediaMetadata`/`brief`，streamed track 两者皆空 → 回退到标题。**修**：`createStreamedTrack` 把 `streamMeta` 镜像进 `mediaMetadata`（`artists`/`album`/`parser:"manual"`）→ 所有现有显示面零改动即生效（与上传 track 同款）。HMR 生效。
- **封面要下载持久化**（用户要求「音频和封面都下载下来」）→ `playStreamedHit` 加 best-effort `downloadStreamedCover`：经 `mediaProxyUrl` 拉封面字节（剥 referer + ACAO）→ `setTrackCover` 存 blob → `coverBlobId` 填上 → 之后走本地 blob URL，不再依赖代理/referer、可离线。音频持久化（缓存）仍属 Phase 5。
...
#### Phase 3 Checklist
- [x] 加密单测：weapi/eapi 对拍 **node:crypto**（独立 AES+MD5）+ weapi 往返；比对 NeriPlayer 输出更强的独立验证。
- [x] `parsePlayback` 分支单测（200/301/404/fee/freeTrialInfo）+ 源 provider 301/VIP 分流。
- [ ] 真机：登录后可放无损；未登录退化/提示。
- [ ] 音质降级链命中下一档单测。
