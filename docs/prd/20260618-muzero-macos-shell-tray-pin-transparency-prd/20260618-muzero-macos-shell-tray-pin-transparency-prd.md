...
- **Shadow-residue ghost (fixed 2026-06-19 per feedback):** after the stale-frame repaint fix, a *shadow-shaped* residue still smeared from both the lyrics and the control bar. Root cause: `text-shadow` / `drop-shadow` / `box-shadow` / `backdrop-filter` are **blurred regions painted outside the element box** — on a transparent window Chromium doesn't clear them as the lyrics scroll/cascade or the bar fades, so the soft halos leave a "残影". Fix: strip those effects in the transparent capture — global CSS under `[data-muzero-lyrics-overlay="true"] .lyrics-immersive-surface` ([styles.css](../../../src/styles.css)) zeroes `text-shadow`/`box-shadow`/`backdrop-filter` (and `filter` on karaoke `[data-word]` spans, without touching the cascade engine's per-row blur), plus the control bar drops its own `shadow-lg` + `backdrop-blur` ([floating-unpin-button.tsx](../../../src/components/player/floating-unpin-button.tsx)). Solid text fill + pill background still repaint cleanly; a non-blurred text **stroke** (painted at the glyph edge) is kept for legibility and never ghosts. The control-bar fade also gets the repaint nudge ([App.tsx](../../../src/App.tsx) `useTransparentWindowRepaint(..., { fadeMs: 200 })`).
