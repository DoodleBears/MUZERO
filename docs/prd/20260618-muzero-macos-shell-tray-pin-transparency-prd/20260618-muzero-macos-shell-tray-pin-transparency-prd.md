...
- **Unfocused-window trail (fixed 2026-06-19 per feedback):** once shadows/filters were stripped, QA found the residue is **focus-dependent** — clean while focused, trails the moment the window is unfocused (the normal state for a live desktop overlay). macOS stops compositing an unfocused transparent window, so moving lyrics stop getting cleared. **A confirmed full restart with `backgroundThrottling:false` + per-frame `webContents.invalidate()` did NOT fix it** → `invalidate()` is too weak to force an unfocused recomposite. What *does* work is the same recomposite that **focusing** triggers, replicated via a tiny imperceptible **`setOpacity` nudge** (0.999 ↔ 1.0). Fix: (1) keep `backgroundThrottling: false` ([main.cjs](../../../electron/main.cjs)) so the renderer keeps producing frames while unfocused; (2) a **main-process** loop (`muzero:window:setContinuousRepaint`, [ipc.cjs](../../../electron/ipc.cjs)) that, while the *playing* lyrics-only capture is active, invalidates + opacity-nudges the window at ~30fps — immune to renderer/focus throttling. [`useContinuousTransparentRepaint`](../../../src/hooks/use-transparent-window-repaint.ts) just toggles it on/off ([App.tsx](../../../src/App.tsx), gated `lyricsOnlyIdle && isPlaying`); opacity is reset to 1 on stop. Tests: [use-transparent-window-repaint.test.tsx](../../../src/hooks/use-transparent-window-repaint.test.tsx).
...
### 7.1 Known limitation → future work: canvas lyrics overlay

The macOS transparent lyrics overlay leaves **scroll "残影" / afterimage trails** while the lyrics animate (worse when the window is unfocused). Root cause: Electron renders animated DOM content into a macOS transparent window's backing **without clearing previous frames** — a Chromium/macOS compositor limitation, NOT a MUZERO rendering bug. **Confirmed dead-ends** (all real‑Mac tested, none fixed it): `webContents.invalidate()` (one‑shot + per‑frame), `setOpacity` recomposite nudge, `backgroundThrottling:false`, 1/255‑alpha backing, stripping shadow/`filter`/`will-change`, and **framed‑vs‑frameless** (both trail — so the frame is not the cause; Windows is trail‑free because DWM composites transparent surfaces differently). See the [Document Change Log](#11-document-change-log) for the full sequence.

**Durable fix (future PRD/phase):** render the overlay lyrics into a single self‑clearing `<canvas>` (clear + redraw each frame) — one composited layer that can't accumulate, the approach native/AMLL‑style lyric apps use precisely for transparent overlays. Substantial build (text layout/wrap, scroll, karaoke word‑fill, translations); scoped separately. Until then the trail is an **accepted limitation** of the transparent capture.
...
|---|----------|--------|----------|
| 1 | For Issue #3, does `transparent: true` + `titleBarStyle: "hiddenInset"` keep working native traffic lights + window shadow on macOS, or must we move to `frame:false` + the standalone macOS controls from Issue #2? | ✅ Resolved (revised 2026-06-19) | **Took the `frame:false` fallback.** Real-Mac QA proved a FRAMED transparent macOS window (`hiddenInset`) leaks scroll trails that no compositor workaround fixed (invalidate / setOpacity / backgroundThrottling / alpha backing / shadow+filter strips all failed). Windows never trailed *because* it's frameless. macOS is now frameless + pure-transparent, identical to Windows. **Cost: native traffic lights are gone** → app-drawn controls needed (follow-up: extend the custom control cluster to macOS). |
...
| 2026-06-19 | MUZERO | **Cleaned up failed trail experiments (accept limitation; canvas later)**: reverted the confirmed-non-working trail fixes — main-process `setContinuousRepaint`/`setOpacity` loop (IPC+bridge+hook), `backgroundThrottling:false`, and the CSS shadow/`filter`/`will-change` strips (restored normal lyric shadow/blur rendering). **Kept** what works: transparent window, background-ghost one-shot `repaint`, locked-overlay reveal precision, and the control‑bar shadow/blur removal. The macOS transparent-overlay scroll trail is accepted as a **known limitation**; durable fix = a self-clearing **canvas** lyrics layer, tracked as future work (§7). 2949 tests green. |
| 2026-06-19 | MUZERO | **Frameless macOS REVERTED** — real-Mac test showed frameless did NOT fix the trails either, so the frame is not the cause. Restored macOS to framed + `hiddenInset` + native traffic lights (better UX since nothing about the window config helps). Confirms the trail is a deeper macOS limitation; the only remaining fix is a self-clearing canvas lyrics layer (or accept the limitation). |
| 2026-06-19 | MUZERO | **Windows-vs-macOS investigation → macOS made FRAMELESS**: per QA, Windows never had the trail; macOS did — and both run transparent. The only structural difference was `frame`: Windows `frame:false` (frameless), macOS `frame:true` + `titleBarStyle:"hiddenInset"` (framed, for native traffic lights). A FRAMED transparent macOS window is composited differently and doesn't clear moving content; a frameless one does. Made macOS frameless + pure `#00000000`, identical to Windows ([window-chrome.cjs](../../../electron/window-chrome.cjs)) — **enacts the Q1 fallback; native traffic lights dropped** (app-drawn controls instead). Reverts the `#01010101` backing. |
| 2026-06-19 | MUZERO | **Shadow-render investigation + `will-change` strip**: traced the shadow path (lyric-style `resolveTextShadow` → inline `textShadow` classic / word-span `drop-shadow` karaoke); confirmed the capture strip already removes it (selector-`!important` beats inline) — so the 残影 is frozen frames of the dimmed inactive lines, not a CSS shadow, in BOTH modes. Each line carries a permanent `will-change: transform` GPU layer the transparent window never clears; added `will-change: auto !important` to the capture strip ([styles.css](../../../src/styles.css)) so lines repaint into the shared surface instead of orphaned layers |
| 2026-06-19 | MUZERO | **Unfocused-window trail fix (setOpacity recomposite)**: confirmed full-restart test showed `invalidate()` is too weak to force an unfocused transparent recomposite. Switched to a MAIN-PROCESS loop (`muzero:window:setContinuousRepaint`, [ipc.cjs](../../../electron/ipc.cjs)) that invalidates **+ nudges `setOpacity` (0.999↔1)** at ~30fps while the playing capture is active — the same recomposite focusing triggers, immune to renderer throttling. Hook now just toggles it. Tests rewritten |
