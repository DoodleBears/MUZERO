...
| 2 | Per-Style Tuning Data Model + Resolver | 🔲 Pending | [Phase 2 Checklist](#phase-2-checklist) |
...
**Tasks:**
- [ ] Add `VisualizerStyleTuning` and `visualizerTuningByStyle` to `src/db/types.ts`.
- [ ] Update `DEFAULT_SETTINGS` only if a seeded empty map is valuable; otherwise rely on resolver defaults.
- [ ] Refactor `resolveVisualizerRenderOptions` and `resolveVisualizerAnalyserOptions` to accept resolved per-style tuning.
- [ ] Add helper for saving a patch into the active style's tuning map.
- [ ] Add helper for deleting/resetting the active style's tuning entry so defaults take over.
- [ ] Preserve legacy global fields as fallback.

### Phase 2 Checklist

- [ ] Bars tuning does not change Aura tuning.
- [ ] Waveform density does not reuse Bars bands/octave values unless intentionally copied.
- [ ] Existing users keep their current active-style tuning on first use.
- [ ] Invalid persisted numbers are clamped.
- [ ] Resetting one style does not change any other style's saved tuning.
