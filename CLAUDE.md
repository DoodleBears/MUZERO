...
- 「流光背景（封面取色多色流光）」→ **独立合成层**（**不是**可视化频谱样式、不互斥）：[`now-playing-background.tsx`](src/components/player/now-playing-background.tsx) 里垫在 **背景图片/视频之上、可视化频谱之下**，由 `AppSettings.flowEnabled` 独立开关（+ `flowOpacity`/`flowDim`），渲染走 `VisualizerHost styleId="scene-flow"`（强制 styleId）。`scene-flow` 在 registry 标 `hidden:true` → 不进频谱选择器（[`VISUALIZER_PICKER_META`](src/visualizer/registry.ts)），只作图层。效果 shader 在 [`flow-shaders.ts`](src/visualizer/scene/flow-shaders.ts) `FLOW_FRAGS`（**14 个自研 GLSL，对齐 color4bg 全 14 style**：blob/wave/gradient/geometric 家族，共享 `FLOW_PRELUDE` 的 noise/fbm/`ramp()` 多色映射 + 轻度音频；每效果一 shader 按 `flowEffect` 选、按需编译）。取色多色化在 [`image-palette.ts`](src/lib/image-palette.ts) `extractImagePalette`（零依赖 canvas 量化，top-N 去重）→ [`visualizer-color-store.ts`](src/stores/visualizer-color-store.ts) `palette`；取色源回退（跟随封面 / 无封面→自定义多色，二者同设）唯一裁决在 [`flow-config.ts`](src/lib/flow-config.ts) `resolveFlowColors`/`resolveFlowConfig`。配置走 Settings「流光背景」面板（[`flow-settings.tsx`](src/components/settings/flow-settings.tsx)）。**不引入 color4bg/ogl/node-vibrant**——全自研（PRD `20260611-muzero-immersive-flow-background-prd`）
