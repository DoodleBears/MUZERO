            ...
            <section key={section.category} className="flex flex-col gap-0.5">
              ...
              {rows.map((row) => (
                <ShortcutRow
                  key={row.actionId}
                  row={row}
                  hasOverride={!!overrides && row.actionId in overrides}
                  onRecord={() => setRecording({ actionId: row.actionId, label: td(row.labelKey) })}
                />
              ))}
            </section>
          );
        ...
        {row.editable && (
          <>
            ...
            {hasOverride && (
              <button
                type="button"
                onClick={() => void resetShortcut(row.actionId)}
