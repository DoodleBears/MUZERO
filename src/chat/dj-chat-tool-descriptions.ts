...
const ZH: Record<string, string> = {
  ...
  dj_say:
    "对听众说一句简短、自然的回话（一两句）——说明你做了或即将做什么，用 DJ 的口吻。每当你响应一个语音/口头请求时都调用它，让用户得到可见（并可朗读）的回应。不要念工具机制或 id；保持口语化。",
...
const JA: Record<string, string> = {
  ...
  dj_say:
    "リスナーへ短く自然な返答を（1〜2 文）——何をした/これからするかを DJ の口調で。音声/口頭のリクエストに応えるたびに呼び、可視（必要なら読み上げ）の応答を返す。ツールの仕組みや id は口にせず、会話調で。",
  online_search_tracks:
...
const KO: Record<string, string> = {
  ...
  dj_say:
    "청취자에게 짧고 자연스러운 답변을(한두 문장) — 무엇을 했는지/할 것인지 DJ 말투로. 음성/구두 요청에 응할 때마다 호출해 가시적(원하면 낭독) 응답을 제공. 도구 메커니즘이나 id는 말하지 말고 대화체로.",
  online_search_tracks:
