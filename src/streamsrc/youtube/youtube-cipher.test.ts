...
describe("resolveFormatUrl", () => {
  it("descrambles the signature, attaches it, and transforms n (ciphered format)", async () => {
    ...
    const out = await resolveFormatUrl(format, solvers);
  ...
  it("only transforms n for a non-ciphered (direct url) format", async () => {
    const out = await resolveFormatUrl({ url: "https://cdn.example/vid?itag=140&n=abc" }, solvers);
    expect(new URL(out as string).searchParams.get("n")).toBe("ABC");
  });

  it("leaves a url without an n param untouched aside from the signature", async () => {
    const out = await resolveFormatUrl({ url: "https://cdn.example/vid?itag=140" }, solvers);
    expect(out).toBe("https://cdn.example/vid?itag=140");
  });

  it("returns null when the format has neither url nor a valid cipher", async () => {
    expect(await resolveFormatUrl({}, solvers)).toBeNull();
    expect(await resolveFormatUrl({ signatureCipher: "sp=sig" }, solvers)).toBeNull();
  });
});
